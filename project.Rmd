...
# The reason we why we have chose cut-off value of .75 is because it gives us variables that are strongly correlated with each other
length(high)
```
...
# Here we have applied GLM to our 56 variables, because it must be remembered that our dataset is categorical in nature. 
```
*After having run the regression we have see that most of the variables are significant. This means that the dimension reduction we took earlier is helping us to come up with a good model*
...
*Now we went one step further and applied a step-wise backwards regression which further helped us reduce variables*
