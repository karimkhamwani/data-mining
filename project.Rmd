...
# After removing our suspected predictors we still have strong positive correlation with 99.6% and strong negative corelation with 99.9%, we need to find which of them are highly corelated
```

```{r}
highCor <- findCorrelation(correlationMatrix, cutoff = 0.75, names = TRUE)
highCor
length(highCor)
# there are 34 varaibles which are highcorelated with each other, before droping them we need to see how they are corelated with response variable carvan.
```
...
# high corelation between  Contribution boat policies and Number of car policies, APERSAUT has low corelation with response variable hence as compare to PPERSAUT so we exclude APERSAUT from regression
```


```{r}
library('rlist')
target_cor_df <- data.frame(V86 = cor(train_2[,sort(highCor)], df_train[, "CARAVAN"]))
head(train_2)
...
excludedDummiesAndVariables = list.append(excludedVariables, 'APERSAUT', 'maintype', 'age', 'noofhouses','hasThreeHouseHold', 'moreThanTwoHouse', 'averageFamily', 'loners', 'conservativeFamilies', 'crusingSeniors', 'drivenGrowers', 'grownups', 'framers','livingWell', 'retired', 'successful','MINK7512_c', 'MINK123M_c')

excludedVariables

length(excludedVariables)

# There are 33 varibles which are less correlated with response variable to there is n sense to take them into regression so we will drop variables where corelation coefficent is less than 0.9
```
...
for (i in colnames(train_3)){
    cor_test = cor.test(train_3[, i], train_data_3[, "CARAVAN"])
...
# Summary
# 3 coefficient as positive behavior and 3 has negative behavior against response variable
# lower education is highly signicant it means education is highly weighted against response variable
# PPERSAUT is highly siginicat the more customer has contirubtion towards car policy the more he is likely to buy insurance.
# MINKM30 is not significant it means if customer is earning less than 30k then he might not buy insurance because he can't afford it?
# Median of deviance is -0.2810 which negative and close to zero telling us predictor we choose badly fit the model we need to try with other combination. 
```
...
glm.3 = glm(CARAVAN ~ ., data = df_train, family = binomial(link = "logit"))
glm3.summary <- summary(glm.3)
glm3.summary
```
