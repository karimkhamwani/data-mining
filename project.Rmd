...
# subTypeFactor = factor(df_train$MOSTYPE, levels = c(min(df$subtype) : max(df$subtype)))
# mainTypeFactor = factor(df_train$)

logit.reg = glm(CARAVAN ~ noOfHouseFactor + drivenGrowers + MINKGEM_c, data = df_train, family = binomial (link = "logit"))
