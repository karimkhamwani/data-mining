...
```{r}
library('rlist')
target_cor_df <- data.frame(V86 = cor(df_train[,sort(high)], df_train[, "CARAVAN"]))
...
```{r}
cor_response = data.frame("ind_var" = colnames(train_2), "dep_var" = "CARAVAN", "cor_coeff" = 0, "p_values" = 0)

for (i in colnames(train_2)){
    cor_test = cor.test(train_2[, i], train_2[, "CARAVAN"])
...
```{r}
c = glm(formula = CARAVAN ~ MAANTHUI + MGEMOMV + MGEMLEEF + MOSHOOFD + 
    MGODRK + MGODPR + MGODOV + MGODGE + MRELGE + MRELSA + MFWEKIND + MOPLMIDD, family = binomial(link = "logit"), data = train_2)
# summary(c)
predicted3 = predict(c, over_test, type = "response")
predictedClass3 = ifelse(predicted3>=0.5, 1, 0)
confusionMatrix(as.factor(predictedClass3), as.factor(over_test$CARAVAN), positive = "1")
```
# explanation
...
glm_5 = glm(formula = CARAVAN ~ PBRAND + MOSTYPE + PPERSAUT + MKOOPKLA+MHKOOP, family = binomial(link = "logit"), 
    data = over_train)
...
glm_5$xlevels[["PPERSAUT"]] <- union(glm_5$xlevels[["PPERSAUT"]], levels(over_test$PPERSAUT))

predicted_5 = predict(glm_5, over_test, type = "response")

predictedClass_5 = ifelse(predicted_5>=0.4, 1, 0)

confusionMatrix(as.factor(predictedClass_5), as.factor(over_test$CARAVAN), positive = "1")

# Area under curve
pr <- prediction(predictedClass_5,over_test$CARAVAN)
perf <- performance(pr,measure = "tpr",x.measure = "fpr")
plot(perf) > auc(over_test$CARAVAN,predictedClass_5)
...
accuracy(predictedClass_5, as.numeric(over_test$CARAVAN))
...
```{r}
pred_t <- predict(glm_5, na.action=na.pass)
# accuracy(pred_t, over_test$CARAVAN)
hist(pred_t)
boxplot(pred_t)

##Plotting residual histograms for training and validation data
resid.t<-residuals(glm_5)
hist(resid.t)
```
...
```{r}
# library(pROC)
# r <- roc(over_test$CARAVAN, predictedClass_5)
# plot.roc(r)
# auc(r)
```
...
# Lift chart
```{r}
lift.example <- lift(relevel(as.factor(over_test$CARAVAN), ref="1") ~ predictedClass_5, data = over_test)
...
predictedClass_5_num = as.numeric(predictedClass_5)
gain = gains(actual, predictedClass_5_num)
