...
train_4 = data.frame(train_3[ , !colnames(train_3) %in% c("APERSAUT")])
```
...
```{r}
step.wise1 = glm(CARAVAN ~ ., data = train_4, family = binomial(link = "logit"))
summary(step.wise1)
# step(step.wise1, direction = "both")
```


```{r}
c = glm(formula = CARAVAN ~ MGODRK + MRELSA + MOPLMIDD + MOPLLAAG + 
    MBERHOOG + MBERZELF + MBERMIDD + MBERARBG + MBERARBO + MSKC + 
    MHHUUR + MAUT1 + MAUT2 + MKOOPKLA + PWALAND + PPERSAUT + 
    PFIETS + AWAPART + AWAOREG + ABRAND + APLEZIER + ABYSTAND + 
    MGEMLEEF_c, family = binomial(link = "logit"), data = train_4)
summary(c)
predicted3 = predict(c, df_test, type = "response")
predictedClass3 = ifelse(predicted3>=0.5, 1, 0)
confusionMatrix(as.factor(predictedClass3), as.factor(df_test$CARAVAN), positive = "1")
```
# explanation
